...
try {
    $sql = "SELECT COUNT(DISTINCT a.asset_id) as total_assets, COUNT(c.check_id) as total_checks_24h, SUM(CASE WHEN c.up = 1 THEN 1 ELSE 0 END) as up_checks_24h, AVG(c.response_time) as avg_response_time_24h FROM assets a LEFT JOIN checks c ON a.asset_id = c.asset_id WHERE a.status = 1 AND c.checked_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR)";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    $stats = $stmt->fetch(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    $sql = "SELECT COUNT(DISTINCT a.id) as total_assets, COUNT(uc.id) as total_checks_24h, SUM(CASE WHEN uc.up = 1 THEN 1 ELSE 0 END) as up_checks_24h, AVG(uc.response_time) as avg_response_time_24h FROM assets a LEFT JOIN checks uc ON a.id = uc.asset_id WHERE a.status = 1 AND uc.checked_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR)";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    $stats = $stmt->fetch(PDO::FETCH_ASSOC);
}
...
try {
    $sql = "SELECT a.name, c.up as status, c.error_message, c.checked_at FROM checks c JOIN assets a ON c.asset_id = a.asset_id WHERE c.up != 1 AND c.checked_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR) ORDER BY c.checked_at DESC LIMIT 10";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    $recent_issues = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    $sql = "SELECT a.name, uc.up as status, uc.error_message, uc.checked_at FROM checks uc JOIN assets a ON uc.asset_id = a.id WHERE uc.up != 1 AND uc.checked_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR) ORDER BY uc.checked_at DESC LIMIT 10";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    $recent_issues = $stmt->fetchAll(PDO::FETCH_ASSOC);
}
...
try {
    $sql = "SELECT a.*, c.up as latest_status, c.response_time as latest_response_time, c.checked_at as last_check 
            FROM assets a 
    ...
    $sql = "SELECT a.*, uc.up as latest_status, uc.response_time as latest_response_time, uc.checked_at as last_check 
            FROM assets a 
            LEFT JOIN checks uc ON a.id = uc.asset_id 
            WHERE a.status = 1 
            AND (uc.checked_at = (SELECT MAX(checked_at) FROM checks WHERE asset_id = a.id) OR uc.checked_at IS NULL)
            ORDER BY a.name";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    $assets_status = $stmt->fetchAll(PDO::FETCH_ASSOC);
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <span class="w3-tag w3-round w3-<?= $asset['latest_status'] === 1 ? 'green' : ($asset['latest_status'] === 0 ? 'red' : 'orange') ?>">
                                <i class="fas fa-<?= $asset['latest_status'] === 1 ? 'check' : ($asset['latest_status'] === 0 ? 'times' : 'exclamation') ?>"></i>
