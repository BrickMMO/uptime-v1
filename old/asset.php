...
<div class="w3-container w3-margin-top">
    <div class="w3-card w3-white w3-padding">
        ...
        <?php if (!empty($recent_checks)): 
            ...
            $latest_status = isset($latest['up']) ? $latest['up'] : 'unknown';
        ...
        <div class="w3-row-padding">
            <div class="w3-col l3 m6 s12">
                <div class="w3-center w3-padding">
                    <i class="fas fa-<?= $latest_status === 1 ? 'check-circle' : ($latest_status === 0 ? 'times-circle' : 'exclamation-triangle') ?> 
                              status-<?= $latest_status === 1 ? 'up' : 'down' ?> w3-jumbo"></i>
                    <h4 class="status-<?= $latest_status === 1 ? 'up' : 'down' ?>"><?= $latest_status === 1 ? 'UP' : 'DOWN' ?></h4>
                    <p>Current Status</p>
                </div>
            </div>
            ...
            <table class="w3-table w3-striped w3-bordered">
                ...
                <tbody>
                    <?php foreach (array_slice($recent_checks, 0, 50) as $check): 
                        $c_status = isset($check['up']) ? $check['up'] : 'unknown';
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="w3-tag w3-round w3-<?= $c_status === 1 ? 'green' : ($c_status === 0 ? 'red' : 'orange') ?>">
                                <i class="fas fa-<?= $c_status === 1 ? 'check' : ($c_status === 0 ? 'times' : 'exclamation') ?>"></i>
                                <?= $c_status === 1 ? 'UP' : ($c_status === 0 ? 'DOWN' : 'UNKNOWN') ?>
                            </span>
                        </td>
                        <td><?= $c_response ?></td>
