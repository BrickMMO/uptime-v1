...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'run_check') {
        ...
        try {
            $stmt = $pdo->prepare("DELETE FROM checks WHERE checked_at < DATE_SUB(NOW(), INTERVAL 30 DAY)");
            $stmt->execute();
...
$stmt = $pdo->prepare("
    SELECT 
        (SELECT COUNT(*) FROM assets WHERE status = 1) as active_assets,
        (SELECT COUNT(*) FROM checks) as total_checks,
        (SELECT COUNT(*) FROM checks WHERE checked_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR)) as checks_24h,
...
$stmt = $pdo->prepare("
    ...
    WHERE table_schema = ? 
    AND table_name IN ('assets', 'checks', 'page_errors', 'admins')
    ORDER BY (data_length + index_length) DESC
");
$stmt->execute([DB_NAME]);
$table_stats = $stmt->fetchAll(PDO::FETCH_ASSOC);

$stmt = $pdo->prepare("SELECT MAX(checked_at) as last_check FROM checks");
$stmt->execute();
